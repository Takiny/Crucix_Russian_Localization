...
import { translateSynthesizedData } from './lib/llm/translate.mjs';
...
async function runSweepCycle() {
  ...
  try {
    ...
    // 5.5 Optional: translate content (news/OSINT/ideas/etc.) via LLM with cache
    // Run AFTER LLM idea generation so idea cards are translated too.
    // Run AFTER delta so translation doesn't affect change tracking.
    if (config.contentTranslation?.enabled && config.contentTranslation?.targetLang) {
      try {
        console.log(`[Crucix] Translating content → ${config.contentTranslation.targetLang} (LLM)...`);
        const stats = await translateSynthesizedData(synthesized, llmProvider, config.contentTranslation);
        console.log(`[Crucix] Content translation: ${stats.translated} translated, ${stats.cached} cached, ${stats.batches} batches`);
      } catch (trErr) {
        console.error('[Crucix] Content translation failed (non-fatal):', trErr.message);
      }
    }
