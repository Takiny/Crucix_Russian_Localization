...
export default {
  ...
  // Optional: LLM-driven translation of dashboard content (news/OSINT/etc.)
  // UI language is controlled separately via CRUCIX_LANG.
  contentTranslation: {
    enabled: String(process.env.CONTENT_TRANSLATE || '').toLowerCase() === 'true',
    targetLang: (process.env.CONTENT_LANG || '').toLowerCase().slice(0, 2) || null, // e.g. "ru"
    maxItemsPerRequest: parseInt(process.env.CONTENT_TRANSLATE_BATCH || '40'),
    maxCharsPerItem: parseInt(process.env.CONTENT_TRANSLATE_MAX_CHARS || '240'),
  },
